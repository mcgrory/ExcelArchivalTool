...
'
' This script is used by the file ExcelArchivalTool.HTA. It is capable of functioning as a standalone
' script, if desired, by entering the following command into a Windows command prompt:
'
' <path of ToolLogic.vbs> <path to Excel file or directory holding Excel files> <path to destination directory> <boolean1> <boolean2> <boolean3> <boolean4>
'
' If the boolean arguments are true, then the corresponding conversion will be executed:
' 	boolean1 = raw spreadsheet data to CSV
' 	boolean2 = cell formulae logged into TXT file
' 	boolean3 = spreadsheet charts/figures exported as PNG images
' 	boolean4 = spreadsheet exported as HTML copy (preserves spreadsheet formatting, styling, etc.)
'
' 
...
Private Function extractCharts(wkbook,fileBaseName,fileExtension)
	...
	For each wksheet in currBook.Worksheets
		...
		If usedColumn > 1 Then
			column = usedColumn - 1
			Set colDeleteRange = wksheet.Range("A1:" & converToLetter(column) & "1")
...
Private Function convertAddress(column,row,rangeUsed,includeAbsolutes)
	...
	If includeAbsolutes Then
		convertAddress = "$" & converToLetter(column - firstColumn + 1) & "$" & (row - firstRow + 1)
	Else
		convertAddress = converToLetter(column - firstColumn + 1) & (row - firstRow + 1)
	End If
...
Function converToLetter(colNum)
	Dim quotient
	Dim remainder
	If colNum <= 0 Then
		converToLetter = ""
	ElseIf colNum <= 26 Then
		converToLetter = Chr(colNum + 64)
	Else
		...
		If remainder = 0 Then
			...
			remainder = 26
		End If
		converToLetter = converToLetter(quotient) & converToLetter(remainder)
	End If
End Function
...
If Wscript.Arguments.Count < 6 Then
	Wscript.Echo "Not enough arguments. Closing Script."
	Wscript.Quit
End If
...
getData = Wscript.Arguments(2)
getFormulas = Wscript.Arguments(3)
getChartsFigures = Wscript.Arguments(4)
getHTMLRepresentation = Wscript.Arguments(5)		
